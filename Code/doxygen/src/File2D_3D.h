...
class vector2 {
	...
	///Constructor for a vector2 tuple
		...
		void set(int x, int y, string name_in){
			a = x; 
			b = y;
			name = name_in;
		}
...
class File2D{
	...
	void Makenode2D(View_Node2D node); 
	///make a solid line between two nodes
	void SolidLine2D(View_Node2D node1, View_Node2D node2);
	///make a dashed line between two nodes
	void DashedLine2D(View_Node2D node1, View_Node2D node2);
	///Get Labels for Unlabled 2D File
	void getLabels();
	///Delete a selected node from the 2D File
	void deleteNode(View_Node2D node);
};
...
class File3D{
	...
	void Makenode3D(View_Node3D node); 
	///Make solid line between two nodes
	void MakeEdge3D(View_Node3D node1, View_Node3D node2);
	...
	void deleteNode3D(View_Node3D node);
};
